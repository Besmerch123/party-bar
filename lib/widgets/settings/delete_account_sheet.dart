...
///
/// [partyService] is a test seam only: production always leaves it null and
/// gets a real [PartyService]. Nothing in the app passes it.
Future<bool?> showDeleteAccountSheet(
  ...
  return showAppSheet<bool>(
    context,
    (context) => _DeleteAccountSheet(partyService: partyService),
    useSafeArea: false,
  );
}

class _DeleteAccountSheet extends StatefulWidget {
  const _DeleteAccountSheet({this.partyService});
...
class _DeleteAccountSheetState extends State<_DeleteAccountSheet> {
  ...
  late final PartyService _partyService = widget.partyService ?? PartyService();
  ...
  late final Future<int> _presetCount = MenuPresets.list().then(
    (p) => p.length,
  );
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      ...
      child: Container(
        ...
        // A bumped text scale or a small phone can make the manifest, the
        // confirm field and both buttons taller than the sheet's own share of
        // the screen — scroll rather than overflow past the bottom edge.
    ...
    return ClipRRect(
      ...
      child: Column(
        children: [
          _row(
            Icons.history,
            l10n.settingsDeleteManifestNights(nights, poured),
          ),
            ...
            Expanded(
              child: Text(
                ...
                style: AppTypography.meta.copyWith(
                  ...
                  color: dim
                      ? AppColors.inkMeta
                      : AppColors.ink.withValues(alpha: .8),
                ),
              ),
            ),
          ],
